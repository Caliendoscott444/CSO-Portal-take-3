// supabase/functions/_shared/amendments.ts
//
// Full text of all 27 Amendments to the U.S. Constitution.
// Public domain U.S. government text — transcribed from the National
// Archives' official transcription (archives.gov/founding-docs).
//
// This file REPLACES the previous amendments.ts. The prior version was
// missing some amendments; this one has been checked against the
// National Archives transcription end-to-end (all 27, I-XXVII).

export interface Amendment {
  number: number;       // 1-27
  roman: string;        // "I", "II", ... "XXVII"
  title: string;         // short common name, for the summary embed
  ratified: string;      // ratification date, as given by the Archives
  text: string;          // full official text, including Section headers where present
}
...
export const AMENDMENTS: Amendment[] = [
  ...
  },
  {
    number: 6,
    roman: "VI",
    title: "Right to Speedy Trial, Confrontation of Witnesses",
    ratified: "December 15, 1791",
  ...
  },
  {
    number: 7,
    roman: "VII",
    title: "Trial by Jury in Civil Cases",
    ratified: "December 15, 1791",
  ...
  },
  {
    number: 10,
    roman: "X",
    title: "Rights Reserved to States or the People",
    ratified: "December 15, 1791",
  ...
  },
  {
    number: 18,
    roman: "XVIII",
    title: "Prohibition of Alcohol (repealed by the 21st Amendment)",
  ...
  },
  {
    number: 20,
    roman: "XX",
    title: "Presidential Term and Succession",
  ...
  },
  {
    number: 22,
    roman: "XXII",
    title: "Two-Term Limit on President",
  ...
  },
  {
    number: 23,
    roman: "XXIII",
    title: "Presidential Vote for Washington D.C.",
  ...
  },
  {
    number: 26,
    roman: "XXVI",
    title: "Voting Age Set to 18 Years",
...
/**
 * Builds the full plain-text document containing all 27 amendments,
 * formatted for delivery as a single Discord file attachment.
 */
export function buildAmendmentsDocument(): string {
  const header =
    "THE AMENDMENTS TO THE UNITED STATES CONSTITUTION\n" +
    "Full text of all 27 ratified amendments\n" +
    "=".repeat(60) + "\n\n";

  const body = AMENDMENTS.map((a) => {
    return (
      `AMENDMENT ${a.roman} — ${a.title}\n` +
      `Ratified: ${a.ratified}\n\n` +
      `${a.text}\n`
    );
  }).join("\n" + "-".repeat(60) + "\n\n");
...
/**
 * Short summary list (number + title only) for the embed sent alongside
 * the attachment, so the DM reads as more than just a bare file.
 */
export function buildAmendmentsSummaryLines(): string[] {
  return AMENDMENTS.map((a) => `**${a.roman}.** ${a.title} (${a.ratified})`);
}
