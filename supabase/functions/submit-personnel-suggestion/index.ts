...
serve(async (req) => {
  ...
  try {
    ...
    const CHANNEL_ID = Deno.env.get("PERSONNEL_SUGGESTIONS_CHANNEL_ID");

    if (!BOT_TOKEN || !CHANNEL_ID) {
      return new Response(
        JSON.stringify({ error: "Missing DISCORD_BOT_TOKEN or PERSONNEL_SUGGESTIONS_CHANNEL_ID" }),
    ...
    const embed = {
      author: {
        name: `${orgAbbr} Personnel Suggestions`,
        icon_url: "https://cso-corporations.vercel.app/CSO_CORPORATION_LOGO_1-2.png",
      },
      title: "📋 New Personnel Suggestion Submitted",
